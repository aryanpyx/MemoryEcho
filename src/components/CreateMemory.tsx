...
export function CreateMemory({ onClose }: CreateMemoryProps) {
  const [isClosing, setIsClosing] = useState(false);
  const [showLocationPicker, setShowLocationPicker] = useState(false);
  const [formData, setFormData] = useState({
    title: "",
    content: "",
    type: "event" as "event" | "photo" | "music" | "calendar" | "video" | "thought",
    tags: "",
    importance: 5,
    ...
    // Thought fields
    mood: "peaceful" as "happy" | "sad" | "excited" | "anxious" | "peaceful" | "frustrated" | "grateful" | "nostalgic",
    isPrivate: false,
    thoughtCategory: "reflection" as "reflection" | "idea" | "dream" | "goal" | "worry" | "gratitude",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let videoThumbnailStorageId = undefined;

      // Handle photo upload if it's a photo memory
      ...
      // Handle video upload if it's a video memory
      if (formData.type === "video" && formData.selectedVideo) {
        const postUrl = await generateUploadUrl();
        const result = await fetch(postUrl, {
          method: "POST",
          headers: { "Content-Type": formData.selectedVideo.type },
          body: formData.selectedVideo,
        });
        const json = await result.json();
        if (!result.ok) {
          throw new Error(`Upload failed: ${JSON.stringify(json)}`);
      ...
      await createMemory({
        ...
        // Video fields
        videoStorageId,
        videoDuration: formData.type === "video" ? formData.videoDuration : undefined,
        ...
        // Thought fields
        mood: formData.type === "thought" ? formData.mood : undefined,
        isPrivate: formData.type === "thought" ? formData.isPrivate : undefined,
        thoughtCategory: formData.type === "thought" ? formData.thoughtCategory : undefined,
      });
  ...
  const extractMusicInfo = (url: string) => {
    // Simple YouTube URL parsing
    if (url.includes('youtube.com') || url.includes('youtu.be')) {
      const videoId = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\n?#]+)/);
      if (videoId) {
        // In a real app, you'd use YouTube API to get title/artist
        setFormData(prev => ({ 
          ...prev, 
          musicTitle: "YouTube Video",
          musicArtist: "Unknown Artist"
        }));
      }
    }
    // Add more music platform parsing as needed
  };
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl transition-all duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
      <div className={`relative max-w-2xl w-full max-h-[90vh] overflow-y-auto bg-gradient-to-br from-slate-900/95 to-purple-900/95 rounded-3xl border border-cyan-500/30 shadow-2xl transition-all duration-500 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}>
        ...
        <div className="p-8 pb-6">
          ...
          <p className="text-slate-300">Capture a moment that matters to you</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(["event", "photo", "music", "calendar", "video", "thought"] as const).map((type) => (
                <button
                  key={type}
                  type="button"
              ...
              <div className="border-2 border-dashed border-purple-400/50 rounded-xl p-6 text-center hover:border-purple-400 transition-colors">
                ...
                <label htmlFor="photo-upload" className="cursor-pointer">
                  {formData.selectedImage ? (
                    ...
                    <div>
                      <div className="text-4xl mb-2">📸</div>
                      <p className="text-purple-300 font-medium">Click to upload a photo</p>
          ...
          {formData.type === "video" && (
            <div>
              <label className="block text-white font-semibold mb-2">Upload Video</label>
              <div className="border-2 border-dashed border-red-400/50 rounded-xl p-6 text-center hover:border-red-400 transition-colors">
                <input
                  type="file"
                  accept="video/*"
                  onChange={(e) => setFormData({ ...formData, selectedVideo: e.target.files?.[0] || null })}
                  className="hidden"
                  id="video-upload"
                />
                <label htmlFor="video-upload" className="cursor-pointer">
                  {formData.selectedVideo ? (
                    <div>
                      <div className="text-4xl mb-2">✅</div>
                      <p className="text-red-300 font-medium">{formData.selectedVideo.name}</p>
                      <p className="text-slate-400 text-sm">Click to change video</p>
                    </div>
                  ) : (
                    <div>
                      <div className="text-4xl mb-2">🎬</div>
                      <p className="text-red-300 font-medium">Click to upload a video</p>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value) extractMusicInfo(e.target.value);
                  }}
                  className="w-full px-4 py-3 bg-black/30 border border-white/20 rounded-xl text-white placeholder-slate-400 focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 outline-none transition-all duration-300"
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {(["happy", "sad", "excited", "anxious", "peaceful", "frustrated", "grateful", "nostalgic"] as const).map((mood) => (
                    <button
                      key={mood}
                      type="button"
                    ...
                    >
                      <div className="text-xl mb-1">{getMoodIcon(mood)}</div>
                      <div className="text-xs font-medium text-white capitalize">{mood}</div>
                    </button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <select
                  value={formData.thoughtCategory}
                  onChange={(e) => setFormData({ ...formData, thoughtCategory: e.target.value as any })}
                  className="w-full px-4 py-3 bg-black/30 border border-white/20 rounded-xl text-white focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 outline-none transition-all duration-300"
                >
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  className="w-4 h-4 text-indigo-600 bg-black/30 border-white/20 rounded focus:ring-indigo-500"
                />
                <label htmlFor="private-thought" className="text-white font-medium">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder={formData.type === "thought" ? "What's on your mind?" : "Describe your memory in detail..."}
              required
            />
          </div>
